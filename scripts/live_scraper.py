...
# Canonical search URL — page 1.  All subsequent pages are generated by build_page_url()
# rather than reading the "Next page" button from Zillow, which can silently mutate the
# map bounds or filter state as the user navigates around the site.
# Filter summary: $350k–$710k, 3+ bed, 2+ bath, 1750+ sqft, 1.5+ acres (65,340 sqft lot)
SEARCH_URL = 'https://www.zillow.com/homes/for_sale/?searchQueryState=%7B%22pagination%22%3A%7B%7D%2C%22isMapVisible%22%3Atrue%2C%22mapBounds%22%3A%7B%22west%22%3A-130.02589725712753%2C%22east%22%3A-117.23781131962754%2C%22south%22%3A37.646090441738295%2C%22north%22%3A50.230773715727246%7D%2C%22mapZoom%22%3A6%2C%22customRegionId%22%3A%227ca8b68bbaX1-CRlexe5hovo8mh_116nid%22%2C%22filterState%22%3A%7B%22sort%22%3A%7B%22value%22%3A%22globalrelevanceex%22%7D%2C%22price%22%3A%7B%22min%22%3A350000%2C%22max%22%3A710000%7D%2C%22mp%22%3A%7B%22min%22%3Anull%2C%22max%22%3A3406%7D%2C%22beds%22%3A%7B%22min%22%3A3%2C%22max%22%3Anull%7D%2C%22baths%22%3A%7B%22min%22%3A2%2C%22max%22%3Anull%7D%2C%22tow%22%3A%7B%22value%22%3Afalse%7D%2C%22mf%22%3A%7B%22value%22%3Afalse%7D%2C%22con%22%3A%7B%22value%22%3Afalse%7D%2C%22land%22%3A%7B%22value%22%3Afalse%7D%2C%22apa%22%3A%7B%22value%22%3Afalse%7D%2C%22apco%22%3A%7B%22value%22%3Afalse%7D%2C%22lot%22%3A%7B%22min%22%3A65340%2C%22max%22%3Anull%7D%2C%22sqft%22%3A%7B%22min%22%3A1750%7D%7D%2C%22isListVisible%22%3Atrue%2C%22usersSearchTerm%22%3A%22%22%7D'
...
# Set to True to enable random mouse-wander movements between actions.
# Disabled by default — the movements can accidentally zoom Zillow's map panel.
ENABLE_MOUSE_WANDER = False

# Hardcoded search criteria — listings outside these bounds are skipped before any navigation or AI work.
CRITERIA_MIN_PRICE  = 350_000
CRITERIA_MAX_PRICE  = 710_000
CRITERIA_MIN_BEDS   = 3
CRITERIA_MIN_BATHS  = 2.0
CRITERIA_MIN_SQFT   = 1_750          # home interior sqft
CRITERIA_MIN_ACRES  = 1.5            # lot size in acres
CRITERIA_MIN_LOT_SQFT = int(CRITERIA_MIN_ACRES * 43_560)  # 65,340 sqft
...
def save_checkpoint(
    ...
    last_processed_zpid: Optional[int] = None,
) -> None:
    ...
    try:
        with open(CHECKPOINT_FILE, 'w') as f:
            json.dump({
                ...
                "last_processed_zpid": last_processed_zpid,
...
def mouse_wander(page, steps: int = 3) -> None:
    ...
    No-op when ENABLE_MOUSE_WANDER is False.
    """
    if not ENABLE_MOUSE_WANDER:
        return
    try:
...
def pause_for_challenge_resolution(page, context_label: str) -> bool:
    ...
    solver_candidates = [
        Path(__file__).resolve().parent / "solve_captcha.py",
        REPO_ROOT / "solve_captcha.py",
        REPO_ROOT / "tools" / "captcha" / "solve_captcha.py",
    ]
    solver_path = next((str(path) for path in solver_candidates if path.exists()), None)
    ...
    try:
        if solver_path and os.path.exists(solver_path):
            ...
            if VERBOSE_LOGGING:
                print(
                    "⚠️ Auto-solver not found. Checked: "
                    + ", ".join(str(path) for path in solver_candidates)
                )
...
EXCLUDED_NON_FOR_SALE_STATUSES = {
...
_EXCLUDED_NON_FOR_SALE_STATUSES_NORMALIZED = {
    status.casefold().strip() for status in EXCLUDED_NON_FOR_SALE_STATUSES
}

def _is_inactive_status(status: Optional[str]) -> bool:
    """Return True when status is not actively for sale (aligned with ai_ranker)."""
    if not status:
        return False
    ...
    if not normalized:
        return False
    return (
        normalized in _EXCLUDED_NON_FOR_SALE_STATUSES_NORMALIZED
        or normalized.startswith("sold for ")
    )
...
def refresh_existing_listing_status(
    ...
    Non-for-sale listings are processed quickly — the page is loaded just long
    enough to read the HTML, then we move on immediately.
    """
    try:
        if VERBOSE_LOGGING:
...
def update_properties_table(
    ...
    Returns (success, photo_count, status) where photo_count is needed for image extraction.
    """
    try:
        ...
        return True, photo_count, property_data.get('status')
...
def process_listing(page, zpid: int, listing_url: str, conn: duckdb.DuckDBPyConnection) -> bool:
    ...
    try:
        ...
        # Hard-coded criteria guard — bail out early before any expensive work if the listing
        # doesn't meet our search criteria.  This is a safety net for cases where Zillow's
        # search filters leak a result that is outside our target range.
        quick_data = extract_property_data_from_html(listing_html)
        quick_price    = quick_data.get('price')
        quick_beds     = quick_data.get('beds')
        quick_baths    = quick_data.get('baths')
        quick_sqft     = quick_data.get('home_size_sqft')
        quick_lot_sqft = quick_data.get('lot_size_sqft')
        criteria_fail_reasons = []
        if quick_price is not None:
            if quick_price < CRITERIA_MIN_PRICE or quick_price > CRITERIA_MAX_PRICE:
                criteria_fail_reasons.append(
                    f"price ${quick_price:,} outside [${CRITERIA_MIN_PRICE:,}–${CRITERIA_MAX_PRICE:,}]"
                )
        if quick_beds is not None and quick_beds < CRITERIA_MIN_BEDS:
            criteria_fail_reasons.append(f"beds {quick_beds} < {CRITERIA_MIN_BEDS}")
        if quick_baths is not None and quick_baths < CRITERIA_MIN_BATHS:
            criteria_fail_reasons.append(f"baths {quick_baths} < {CRITERIA_MIN_BATHS}")
        if quick_sqft is not None and quick_sqft < CRITERIA_MIN_SQFT:
            criteria_fail_reasons.append(f"sqft {quick_sqft:,} < {CRITERIA_MIN_SQFT:,}")
        if quick_lot_sqft is not None and quick_lot_sqft < CRITERIA_MIN_LOT_SQFT:
            quick_acres = quick_lot_sqft / 43_560
            criteria_fail_reasons.append(
                f"lot {quick_acres:.2f} acres < {CRITERIA_MIN_ACRES} acres"
            )
        if criteria_fail_reasons:
            print(
                f"    -> ⛔ ZPID {zpid} fails criteria — skipping AI analysis: "
                + "; ".join(criteria_fail_reasons)
            )
            return True
        ...
        success, photo_count, extracted_status = update_properties_table(conn, zpid, listing_url, listing_html)
        if not success:
            print("    -> ⚠️ Could not update properties table. Continuing with feature extraction...")
            photo_count = 0
            extracted_status = None

        if _is_inactive_status(extracted_status):
            if VERBOSE_LOGGING:
                print(
                    f"    -> ⏭️ Skipping non-for-sale listing for ZPID {zpid} "
                    f"(status: {extracted_status!r})"
                )
            return True
        ...
        # Compare downloads against what we intended to download (respecting cap),
        # not against every discovered URL in oversized galleries.
        expected_download_count = min(len(all_image_urls), target_image_count)
        if expected_download_count > 0 and downloaded_count < (expected_download_count - 2):
            print(f"    -> ❌ IMAGE DOWNLOAD FAILURE:")
            print(f"       Found: {len(all_image_urls)} image URLs")
            print(f"       Expected to download: {expected_download_count} images (cap/target)")
            print(f"       Downloaded: {downloaded_count} images")
            print(f"       Failed: {failed_downloads} downloads")
            print("🛑 STOPPING EXECUTION - Too many download failures!")
            raise Exception(
                f"CRITICAL FAILURE: Only downloaded {downloaded_count}/{expected_download_count} expected images for ZPID {zpid}"
            )
...
def update_existing_listings(page, conn: duckdb.DuckDBPyConnection) -> None:
    ...
    A listing is considered "active" when its status is NULL (unknown) or is NOT in
    EXCLUDED_NON_FOR_SALE_STATUSES (plus "Sold for ..."). Listings whose last_updated is
    ...
    rows = conn.execute(f"""
        ...
        WHERE
            ...
            (
                status IS NULL
                OR (
                    UPPER(TRIM(status)) NOT IN ({excluded_statuses_sql})
                    AND UPPER(TRIM(status)) NOT LIKE 'SOLD FOR %'
                )
            )
...
def build_page_url(page_num: int) -> str:
    """
    Build a Zillow search URL for the given page number by injecting
    {"currentPage": N} into the pagination key of SEARCH_URL's searchQueryState.

    This avoids reading the "Next page" button from the live DOM, which can silently
    carry a mutated searchQueryState (different map bounds, dropped filters) that
    accumulated as the browser navigated individual listings.
    """
    import urllib.parse as _up
    qs_encoded = SEARCH_URL.split("searchQueryState=", 1)[1]
    state = json.loads(_up.unquote(qs_encoded))
    if page_num <= 1:
        state["pagination"] = {}
    else:
        state["pagination"] = {"currentPage": page_num}
    return "https://www.zillow.com/homes/for_sale/?searchQueryState=" + _up.quote(
        json.dumps(state, separators=(",", ":")), safe=""
    )


def get_next_page_url(page, current_page_num: int) -> Optional[str]:
    """
    Return a usable next-page URL or None when the control is missing/disabled.

    We use Zillow's "Next page" button only to *detect* whether a next page
    exists.  The URL is always built from SEARCH_URL via build_page_url() so
    that map bounds and filters are never sourced from the live (potentially
    mutated) browser state.
    """
    try:
        for handle in page.query_selector_all('[title="Next page"]'):
            ...
            if aria_disabled == "true" or not href or href == "#":
                continue
            # A real, enabled href confirms a next page exists.
            # Build the canonical URL from SEARCH_URL — never from the button href.
            next_num = current_page_num + 1
            canonical = build_page_url(next_num)
            if VERBOSE_LOGGING:
                print(f"    -> 🔗 Next page {next_num} — using canonical URL (ignoring Zillow button href).")
            return canonical
    except Exception as e:
        if VERBOSE_LOGGING:
...
def run_live_extraction():
    ...
    with sync_playwright() as p:
        ...
        resume_search_page_url = None
        resume_last_processed_url = None
        resume_last_processed_zpid = None
        completed_successfully = False
        if checkpoint:
            print(f"\n📂 Checkpoint found: page {checkpoint.get('current_page_num', 1)}, "
                  f"listing index {checkpoint.get('resume_listing_index', 0)} "
                  f"(saved {checkpoint.get('saved_at', 'unknown')})")
            if checkpoint.get("search_page_url"):
                print(f"   Saved search URL: {str(checkpoint['search_page_url'])[:120]}")
            answer = input("Resume from checkpoint? (y/n): ").strip().lower()
            if answer == 'y':
                is_resuming = True
                resume_page_num = int(checkpoint.get('current_page_num', 1) or 1)
                resume_listing_index = int(checkpoint.get('resume_listing_index', 0) or 0)
                resume_search_page_url = checkpoint.get("search_page_url")
                resume_last_processed_url = checkpoint.get("last_processed_url")
                resume_last_processed_zpid = checkpoint.get("last_processed_zpid")
        ...
        try:
            ...
            if is_resuming:
                ...
                page.bring_to_front()
                if resume_search_page_url:
                    page.goto(resume_search_page_url, wait_until='load', timeout=60000)
                else:
                    print("⚠️ No saved search URL in checkpoint; falling back to SEARCH_URL.")
                    print("   Resume page alignment may be less precise.")
            ...
            while True:
                ...
                effective_resume_listing_index = resume_listing_index
                if is_resuming and current_page_num == resume_page_num:
                    marker_index = None
                    if resume_last_processed_url:
                        try:
                            marker_index = listing_links_on_page.index(resume_last_processed_url)
                        except ValueError:
                            marker_index = None

                    if marker_index is None and resume_last_processed_zpid:
                        marker_fragment = f"/{int(resume_last_processed_zpid)}_zpid/"
                        for idx, candidate_link in enumerate(listing_links_on_page):
                            if marker_fragment in candidate_link:
                                marker_index = idx
                                break

                    if marker_index is not None:
                        effective_resume_listing_index = marker_index + 1
                        print(
                            f"    ↪️ Resume marker matched listing position {effective_resume_listing_index}; "
                            "using marker-based resume."
                        )
                    elif resume_listing_index > 0:
                        print(
                            "    ⚠️ Resume marker not found in current listing set; "
                            "falling back to checkpoint index."
                        )

                # Process each listing
                for i, link in enumerate(listing_links_on_page):
                    # On a resumed run, skip listings we already processed on this page
                    if is_resuming and current_page_num == resume_page_num and i < effective_resume_listing_index:
                    ...
                    # Pre-filter by price using the price already extracted from the search page.
                    # This avoids navigating to listings that are outside our criteria — Zillow
                    # occasionally includes out-of-range results despite having filters applied.
                    if zpid in zpid_to_price:
                        search_page_price = zpid_to_price[zpid]
                        if search_page_price < CRITERIA_MIN_PRICE or search_page_price > CRITERIA_MAX_PRICE:
                            print(
                                f"    -> ⛔ Skipping ZPID {zpid}: price ${search_page_price:,} is outside "
                                f"[${CRITERIA_MIN_PRICE:,}–${CRITERIA_MAX_PRICE:,}]. "
                                "Zillow search filter leak."
                            )
                            continue
                ...
                next_page_url = get_next_page_url(page, current_page_num)
                if next_page_url:
                    print("\n➡️ Opening next page...")
